...
if args.load_unet and args.load_unet_job_id is not None and args.load_unet_ckpt is not None:
    with open("DICF_JOBID.json", 'r') as file:
...
if args.model_name == "stabilityai/stable-diffusion-2":
    ...
    if args.load_unet:
        ...
        pipe = StableDiffusionPipeline.from_pretrained(model_id, unet=unet, scheduler=scheduler, safety_checker=None, torch_dtype=torch.float16)
    else:
        pipe = StableDiffusionPipeline.from_pretrained(model_id, scheduler=scheduler, safety_checker=None, torch_dtype=torch.float16)
if args.model_name == "stabilityai/stable-diffusion-xl-base-1.0":
    if args.load_unet:
        ...
        pipe = DiffusionPipeline.from_pretrained(model_id, unet=unet, torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
    else:
    ...
    if args.load_unet:
        ...
        pipe = StableDiffusionPipeline.from_pretrained(model_id, unet=unet, safety_checker=None, torch_dtype=torch.float16)
    else:
